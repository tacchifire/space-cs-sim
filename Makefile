...
firmware-u01:
	# EX-U01's pair. Both halves are everything the range has deployed - signed reports, a report
	# store, a beacon, authenticated telecommands. What differs is whether the spacecraft says
	# "I heard you", which is the only way an operator sees an uplink that is being denied.
	. $(ENV) && ZEPHYR_EXTRA_MODULES=$(LIBCSP) west build -p always -b $(BOARD) \
	    -d $(OUT)/build-obc-u01-vuln firmware/apps/obc -- \
	    ...
	    -DCUBERANGE_OBC_BEACON=1 -DCUBERANGE_OBC_ACK_COMMANDS=0 \
	    -DEXTRA_CONF_FILE=$(CUBERANGE_REPO)/firmware/apps/obc/pus_auth.conf
	. $(ENV) && ZEPHYR_EXTRA_MODULES=$(LIBCSP) west build -p always -b $(BOARD) \
	    -d $(OUT)/build-obc-u01-hard firmware/apps/obc -- \
	    -DCUBERANGE_OBC_PUS8_LENGTH_CHECK=1 -DCUBERANGE_OBC_REQUIRE_AUTHORITY=1 \
	    -DCUBERANGE_OBC_VERIFY_REPORTS=1 -DCUBERANGE_OBC_REQUIRE_PUS_AUTH=1 \
	    -DCUBERANGE_OBC_SIGN_REPORTS=1 -DCUBERANGE_OBC_REPORT_STORE=1 \
...
firmware-l02:
	...
	# ground asks, which is the only thing that separates a lost report from a taken one.
	. $(ENV) && ZEPHYR_EXTRA_MODULES=$(LIBCSP) west build -p always -b $(BOARD) \
	    -d $(OUT)/build-obc-l02-vuln firmware/apps/obc -- \
	    -DCUBERANGE_OBC_PUS8_LENGTH_CHECK=1 -DCUBERANGE_OBC_REQUIRE_AUTHORITY=1 \
	    -DCUBERANGE_OBC_VERIFY_REPORTS=1 -DCUBERANGE_OBC_REQUIRE_PUS_AUTH=1 \
	    -DCUBERANGE_OBC_SIGN_REPORTS=1 -DCUBERANGE_OBC_REPORT_STORE=0 \
	    -DEXTRA_CONF_FILE=$(CUBERANGE_REPO)/firmware/apps/obc/pus_auth.conf
	. $(ENV) && ZEPHYR_EXTRA_MODULES=$(LIBCSP) west build -p always -b $(BOARD) \
	    -d $(OUT)/build-obc-l02-hard firmware/apps/obc -- \
	    -DCUBERANGE_OBC_PUS8_LENGTH_CHECK=1 -DCUBERANGE_OBC_REQUIRE_AUTHORITY=1 \
	    -DCUBERANGE_OBC_VERIFY_REPORTS=1 -DCUBERANGE_OBC_REQUIRE_PUS_AUTH=1 \
	    -DCUBERANGE_OBC_SIGN_REPORTS=1 -DCUBERANGE_OBC_REPORT_STORE=1 \
	    -DEXTRA_CONF_FILE=$(CUBERANGE_REPO)/firmware/apps/obc/pus_auth.conf
	@ls -l $(OUT)/build-obc-l02-vuln/zephyr/zephyr.elf $(OUT)/build-obc-l02-hard/zephyr/zephyr.elf
