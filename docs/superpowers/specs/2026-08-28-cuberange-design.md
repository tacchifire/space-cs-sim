...
| D23 | libcsp の **split horizon** は、**送出側インタフェースのアドレス**を**受信側インタフェースのサブネット**と比較する（`csp_io.c` に 3 箇所、`csp_iflist.c:15`）。ルータの 2 つのインタフェースをどちらもノード自身のアドレスで登録すると、これは**あらゆる netmask で真**になる——`csp_iflist.c:21` が netmask からマスクを組み、**netmask 0 はマスク 0** を組み、その下では任意のアドレスが任意のアドレスと等しい | **ルータが何も転送しない。** エラーもログ行もカウンタも無い（`iface->drop` すら増えない）。パケットは受信側インタフェースに届いて、そこで止まる。EX-X01 のクロスリンクは最初この状態で、症状は「EPS が何も言わない」——**フレームが届かなかったときと同じ観測**であり、EX-G04 の主題が 1 層下で再現していた | クロスリンク接続点に**固有の CSP アドレス**（`8i+6`。identity.cmake が持つ）と `/5` を与える。送出は既定インタフェース経由で足りる——ノードが**発信する**パケットは `routed_from == NULL` で、`csp_iflist_is_within_subnet` はそれに 0 を返すため split horizon は掛からない |
...
| W32 | 日本語版の文書が英語版と同じ内容を述べている | **3 つの文書が実質的にずれていた。** `ASSURANCE.ja.md` の表は PUS-C を「独立オラクル無し」、FECF を 2 つとし、参照先に存在しないテスト名を挙げ、さらに「ベクタは未コミット、消費するテストも無い」と書いていた——いずれも英語版では解消済み。`SECURITY.ja.md` は英語版が開示している意図的な脆弱性のうち **3 件を欠いていた**（EX-F01 の標的である特権ハンドラ、地上系の無検査インポート、平文の署名鍵）。`README.ja.md` は `make firmware-g01` を欠き、さらに**演習一覧から EX-G01 が丸ごと抜けていた**——次の演習が土台にする当の演習である——のに「攻撃の起点は4つ揃った」と書き続けていた。**検証状況を述べる文書と、脆弱性を開示する文書自身が、日本語読者に対して古い内容を述べていた。**見出し構造しか比較しない検査では 1 件も捕まらない | 3 文書を実態に合わせて書き直した。`test_doc_parity.py` に**表の行数と箇条書きの数**の比較を追加した——翻訳では本文を比較できないが、行や項目の欠落は数えられる。見出しだけでは 3 件とも捕まらず、表の行を足しても README の欠落は捕まらなかった。見出し・表の行・箇条書きの 3 つで、節・行・項目のいずれも黙って消えることは無くなる。これは「両版が同じことを述べている」ことの保証ではなく、テストもそう主張していない。変異テストで、行を 1 つ落としても項目を 1 つ落としても落ちることを確認済み |
| W33 | `make exercise` は動く——学習者が最初に打つコマンドであり、README も演習の README も、そう案内している | **壊れていた。しかも無言で。** シナリオが `$out` の既定値を持たない形に変えたとき、この 1 つのターゲットだけ `$out` を渡すよう直し忘れた。Renode は最初の `LoadELF` で `-e` チェーンを中断し、末尾のコマンドに到達せず、Monitor プロンプトでぶら下がり、**ログは空**になる。設計書自身が記録している最悪の失敗モードが、誰もが最初に打つコマンドで起きていた。加えて隔離も通っておらず、ポート番号も直書きのままだった。`make check` はこのターゲットを触らない——対話的だからである。W25 と同じ形で、**皆が最初に通る経路にだけ検査が無かった** | `$out` を渡し、`$(ISOLATE)` を通し、ポートを `cuberange.ports` から導出するよう修正。`test_paths.py` が **Makefile 内の全 Renode 起動**について、隔離を通ることと `$out` を渡すことを検査する。Python の起動元だけを見ていて Makefile を見ていなかった。3 種類の変異（`$out` 喪失・隔離喪失・ポート直書き）で落ちることを確認済み |
| W34 | `make check` が触らないターゲットも、少なくとも動きはする | **2 つが壊れていた。** `make spike` は生の `subprocess.Popen` で Renode を起動しており（ウォッチドッグも RSS 上限も無い。W25 と同じ形が、修正されないまま残っていた最後の 1 箇所）、隔離も通っていなかった。`make soak-p0` は **存在して以来一度も走れていなかった**——`pytest.ini` の `addopts = -m "not slow"` が唯一のテストを除外し、pytest は「1 deselected」で exit 5、make はテストが壊れたかのようなエラーを出す。CLAUDE.md はこのターゲットを所要時間つきで案内していた | spike を supervisor 経由・隔離下に変更（実測で通過）。soak に `-m slow` を追加（実測 30/30 成功・再試行 0・4 分 00 秒。文書の「約 3.5 分」も実測値に訂正）。`test_supervisor.py` が **AST で** 生の Renode 起動を検出し、`test_paths.py` が「何も選択しない pytest ターゲット」を検出する。前者は 3 版目でようやく変異を検出した——1 版目は `RenodeSupervisor` という文字列がコメントにあるだけで免除し、2 版目は構文エラーのファイルを黙って読み飛ばしていた |
| W35 | 「CI は機構として存在するが、走っているところを誰も見ていない」 | **意図していたより強い意味で真だった。** API が返したのは「実行 0 回」ではなく**「ワークフロー 0 個」**——GitHub は登録すらしていなかった。ワークフローが登録されるのは最初に実行されたときで、`push` は main にしか一致せず、main に `.github` は無く、`workflow_dispatch` は既定ブランチ上のものにしか現れない。押されていないボタンではなく、ボタンが無かった。そこから緑にするまでに 3 つの欠陥が出た。どれも開発機では原理的に出ない——(a) 作業ブランチが push トリガに無い、(b) キャッシュが `~/zephyrproject` と `~/zephyr-sdk` を復元しても、それらを駆動する pip パッケージは復元せず、導入スクリプトはキャッシュミスのときしか走らない（空の実行は通り、温まった実行はすべて `west build` で死ぬ。**2 回目にだけ失敗し、2 回目以降はずっと 2 回目**）、(c) `west` だけ直すと Zephyr 自身の `requirements.txt` が残り同じ失敗が戻る | 作業ブランチをトリガに追加。ダウンロードのみキャッシュし、環境は毎回入れる。**実行 8（`eaf07ce`、main、両ジョブ成功、ゲート 838 秒で `CHECK PASSED`）を観測**したので、CLAUDE.md・README の両言語で一文を昇格した。1 日も早くはない |
| W36 | ランナーでゲートが落ちたのは probe の速度下限（1.5x）のせいだろう | **外れ。** 診断のため probe を「設定どおり」と「下限なし」で 1 回ずつ走らせたところ、**両方通った**（110 秒／111 秒）。2 vCPU のランナーでも下限は満たす。実際の原因は (b)(c) と同じで、`gate` ジョブにも Zephyr の Python 依存が無かったこと。この 8 コア機が 4.3〜5.1x を出すことを知っていたため、**ホストのせいにするのが楽な推測**になっていた | 下限に「ランナー向けの数値」を勘で置かず、2 段構えの診断で確定させたのは正しかった。推測で書いていれば、無実の検査を恒久的に緩めたうえで真因を見逃していた。診断ステップは役目を終えたので撤去した |
| W37 | EX-L01 の再送防止は「対策版でも本来の機能を果たす」ことを検査済み | **検査していたのは送信機が 1 つの場合だけだった。** 再送防止はリンク全体で `last_seq` を 1 つしか持たず、CCSDS 232.0-B-4 と COP-1 が連番を**仮想チャンネルごと**に維持すると定めているのを見ていない（TC フレームの 6 ビット VCID 欄を、デコーダは跨いでいた）。**実測**: 硬化版 COMM に地上局を 2 局つなぐと、2 局目の正規の ping 3 回がすべて `REJECTED replayed frame` として拒否される。運用者はいない攻撃者を探すことになる。EX-L01 の第 3 アサーション（対策版でも新しい指令を受理する）が通っていたのは、送信者が常に 1 つだったからである | `cr_tc_frame_vcid` を追加（C と Python で全域一致を確認）、`CUBERANGE_COMM_ANTIREPLAY_PER_VC` で VC ごとのカウンタに。実測: 2 局で 6 受理 0 拒否、かつ本物の再送は 2/2 拒否。EX-G03 として演習化した |
| W38 | 連番を 255 と名乗れば運用者を締め出せる | **外れ。** 比較は `(int8_t)(seq - last_seq)` で、カウンタが一周できる——つまり順序ではなく **±127 の巡回窓**である。運用者が連番 1 のとき 200 は **+57**、すなわち「先行」と判定されて受理され、運用者は数コマンドで窓へ戻る。最初に書いた攻撃は 200 を使い、運用者は動き続けた | 100 に変更（99 コマンド分締め出せる）。8 ビットに「遥か先」は無く「後ろに回るまでの先」しかない、という事実を演習のヒントと `solve.py` に書いた。EX-L01 の注記「8 ビットは小さい窓」の具体的な帰結である |
| W39 | ノードあたりの費用は 6 マシンを超えると測定していない（3.3 節） | **測定した。** CSP v1 の 5 ビットアドレスが許す上限、4 機 16 ノードまで回る。8 コア x86-64 ホストで全ノードが毎回起動する:  2 機 8 ノード 7.0 秒 4.25x／3 機 12 ノード 8.5 秒 2.90x／4 機 16 ノード 10.0 秒 1.66〜2.52x、最大 RSS 907 MB。**上限はホストではなくアドレス空間**である（少なくともこのホストでは） | `tools/gen_constellation.py` がシナリオを `cuberange.identity` と `ports` から導出する（手書き版と 90 コマンドが順序まで完全一致することを確認し、`test_gen_constellation.py` がドリフトで落ちる）。`make fleet SATS=4` で走る。**ゲートは 2 機**——イメージ 8 個の追加は `make check` に 8 分を足すので、4 機は測定として記録し、ゲートしない。設計が既に抱えている他の測定と同じ扱いである |
| W40 | 権限検査（EX-G02）と再送検査（EX-G03）は、対策として完結している | **どちらの拒否も宇宙機から出ない。** printk は機上コンソールに出るだけで、運用者が持っているのは下り回線である。**実測**: 副局が権限の無い指令を送ると、機上は `not authorised` と記録するが地上が聞くのは **NOTHING**。CRC を壊して届かなかったフレームと**同じ観測**になる。2 つの `mitigation.md` が自分でこの限界を挙げていたが、実装は無かった | ECSS-E-ST-70-41C サービス 1 の受理失敗報告（PUS 1,2）を実装。Request ID は拒否されたパケット自身の先頭 4 オクテットで、`request_id()` が符号化済みパケットの `raw[0..4]` と一致することをテストで検査した。実測: 地上が `refused APID 0x0A9 seq 0: not authorised (to station 0x0043)` を受け取る。EX-G04 として演習化 |
| W41 | `GroundStation.ping()` の「排出」は無害である | **証拠を捨てていた。** 送信前に `link.poll()` の戻り値を分類せず破棄しており、PUS 1,2 は PUS 8 への返事でその間に誰もポーリングしていないため、**宇宙機が「地上に知らせるために」送った報告を、次の ping が捨てていた** | `collect()` を追加し、排出もそこを通す。分類してから捨てる。証拠を落とす排出は排出ではない |
| W42 | 共有コーデックは「fuzz とサニタイザにかけてある」（SECURITY.md 両言語） | **どちらも偽だった。** fuzzer は存在せず——`test_proto.c` は「**later** fuzzed」、`tests/native/Makefile` も「later fuzzed on the host」と書いていた——`asan` ターゲットは在るのに `make check` は `test` しか走らせていなかった（Makefile のコメントは「CI でサニタイズされる」と書いていた）。ワークフローにも fuzz/asan は無い。W23・W24・W26 と同じ形で、§0 の規則が禁じているもの | シード付きランダム fuzzer を実装し、`test` が `asan` と `fuzz` を必ず走らせるようにした（3 つで 1.5 秒）。SECURITY.md 両言語を実態に書き換え、**いつまで偽だったか**も明記した。実測: 3 種 × 2000 万回で違反ゼロ |
| W43 | fuzzer は不変条件を検査していれば欠陥を見つける | **最初の版は 3 変異中 1 件しか捕まえなかった。** 不変条件が `payload_len + header + fecf == len` という**内部整合性**だけで、`payload_len` を `len` から計算している以上、長さ検査を外しても CRC 検査を外しても成り立つ。**「落ちないこと」は床であって試験ではない。** さらに最初の campaign はデフレーマに生フレームを渡しており、一度も動いていなかった（自己検査が「この campaign は何も探索していない」と落として判明した） | 不変条件を実装の言い換えではなく**デコーダの契約**にした——受理したフレームは宣言長＝実長、FECF 剰余ゼロ、VCID は 6 ビット幅、payload はフレーム内側。5 変異（長さ検査・CRC 検査・payload 長・TM の CRC・VCID 幅）すべてを検出することを確認した |
| W44 | C と Python のコーデックは「バイト単位で突き合わせて」ある（CONTRIBUTING.md 両言語） | **突き合わせていたのは符号化側だけだった。** ヘッダが公開している 9 関数のうち `cr_decode_tc_frame`・`cr_decode_tm_frame`・`cr_tc_frame_vcid` の 3 つは一度も比較されていない。**比較が無かったのは、敵性バイト列を解析する側**である。符号化器は自分が組んだフレームしか見ないが、復号器は攻撃者が選んだ列を見る。片方が受理して片方が拒む列があれば、宇宙機は地上局が捨てたものに対して**無言で**動作する。W42 と同じ形——「してある」と書かれた検証が、最も要る場所だけ空だった | 両復号器を **出すものと拒むもの**の両方で比較。入力には切り詰め・0/2/3/末尾のビット反転・長さフィールドの詐称を含めた。VCID は 0/1/2/31/32/63 で一致を確認。さらに `test_every_exported_c_function_is_cross_checked()` を追加し、ヘッダに新しい `cr_*` が生えて比較が無ければ落ちるようにした。4 変異（C だけ長さ検査を外す・C だけ CRC 検査を外す・C だけ VCID を 8 ビットで読む・未照合の関数を足す）すべてを検出 |
| W45 | ルータのインタフェースを 2 つ登録すれば、その間をパケットが流れる | **流れない。** libcsp の split horizon（D23）が止める。しかも**無言で**止める。最初の実装は COMM の両インタフェースをノード自身のアドレス（`8i+5`）で登録し、注入したフレームが EPS に届かなかった。EPS は拒否時に printk するので沈黙は「届いていない」と読めるはずだった——それは EX-G04 が丸ごと扱っている「**拒否と不達は同じ観測**」そのものである。自分で書いた教訓が、自分のデバッグで通用しなかった | `csp_io.c` を読んで原因を特定し、接続点に固有アドレスを与えた。`verify_ex_x01.py` の第 1 テストは**攻撃の前にリンクが通っていること**を主張する。肯定的な観測を持たない試験群は、自分が何も試していないことを報告できない |
| W46 | 演習 README の手順——「`make exercise` で起動し、別の端末で `solve.py`」——は動く | **動かない。** `make exercise` は Renode を**ループバックしかない名前空間**に置き（SAFE_USE.md、2026-09-10 から）、名前空間は起動ごとに新しく作られる。**実測**: 名前空間内で 0.0.0.0:39411 を listen し、ホストから接続して `ConnectionRefusedError: [Errno 111]`。この手順は**14 ファイル**（全演習・両言語）にある。このセッションで私が書いた EX-X01 の README にも書いた——**確かめる前に、周りに合わせて** | `make exercise` が名前空間の中のシェルに入るようにし（`tools/exercise.sh`）、`RUN=` で 1 コマンドを渡す形も用意。**さらに `isolate` を冪等にした**——既にループバック専用名前空間にいれば新しく作らず直接 exec する。これが無いと、EX-G03・EX-G04 が指示する「端末 3 つ」（`make exercise` / `make channel` / `make gs`）は**名前空間 3 つ**になり、互いに見えない range が 3 つできる。README 14 ファイルを**シェルの中で走らせる形**に書き換え、`tests/e2e/test_documented_workflow.py` が`make exercise ... RUN=` を実際に走らせる。3 変異（準備完了を待たない／`RUN=` を無視する／README から `RUN=` を消す）すべてを検出することを確認した |
| W47 | 機上の暗号は「ビルドが通り、自己テストが通れば」動いている | **自己テストを通した当の処理が、リンクを壊していた。** `sizeof(mbedtls_gcm_context)` は **424**（実測）、Zephyr の `CONFIG_MAIN_STACK_SIZE` の既定は **1024**。COMM の `main()` は既に `csp_init` と CAN インタフェース 2 本の登録でその大半を使っており、起動時の自己テストがそこを溢れさせて隣接するカーネルオブジェクトを壊した。症状は **COMM が起動し、printk し、自分の暗号自己テストに合格し、`link_task` を起動し、そして宇宙リンクから 1 オクテットも受け取らない**。フォールト無し、ログ行無し、カウンタ無し。**成功を報告したものが、失敗の原因だった** | 5 つの仮説を実測で殺して到達した——(1) mbedTLS のヒープ設定、(2) `HW_STACK_PROTECTION`／MPU、(3) `link_task` 自身のスタック（1032 バイトのデフレーマを載せている）、(4) SDLS のフレーム形式（**平文フレームを SDLS ビルドに送っても同じ**）、(5) ハーネス（**同じハーネスで平文ビルドは通る**）。`MAIN_STACK_SIZE=3072`、SDLS ビルドの `LINK_STACK` も 3072、`HW_STACK_PROTECTION=y`（**発生時は入っていなかった**）。`tests/e2e/test_sdls_uplink.py` が 4 件を実測し、`MAIN_STACK_SIZE` を 1024 に戻す変異／MAC を検証しない変異／AAD をヘッダだけにする変異の3 つで落ちることを確認した |
| W48 | 「攻撃前に衛星が応答している」という前提条件は、ping 1 回で確かめられる | **リンクの運を、対策の失敗として報告していた。** 5 つの検証スクリプトがそれぞれ `return self.station.ping(timeout=...) is not None` を持ち、`assert r.alive(), "the satellite was not answering before the attack"` として**10 箇所以上**で使っていた。ping 1 回が返らなければ、試験対象の攻撃を送る前にテスト全体が落ちる。2026-09-12 の CI で EX-G01 が実際にこれで落ち、**同じテストの再実行 3 回はすべて通った**。これがいちばん悪い結果である——次の人は緑になるまで再実行し、そのテストが何を言っているかを読むのをやめる | `GroundStation.alive(deadline_s, per_ping_s)` を共有コードに置き、締切までリトライさせた。**2 回目に応答する宇宙機は生きている。締切内に一度も応答しない宇宙機は生きておらず、これはそう言い続ける**——主張が「1 フレーム」から「宇宙機」に変わっただけである。5 本の検証スクリプトと `test_constellation.py` を委譲に変え、`test_no_verifier_decides_liveness_on_a_single_ping` が 6 本目を禁じる。変異 2 つ（`alive()` を単発に戻す／検証スクリプト 1 本だけ単発に戻す）で落ちることを確認した |
| W49 | 共有ヘルパを入れて呼び出し側を書き換えれば、間欠故障は直る | **その書き換えが 3 つのゲートを壊し、壊れ方は「対策が効かなくなった」と区別が付かなかった。** W48 の修正で `GroundStation.alive(deadline_s=...)` を足したが、5 本の検証スクリプトは既に自前の `alive(timeout=15)` と `not alive(timeout=8)` を持っていた。全部 `TypeError` になり、**pytest はそれを前提条件の失敗として報告する**——EX-L01 の 3 件が「衛星が応答しない」と言って落ちた。W48 を直した CI 実行が、そのまま W49 を出した | 引数名を `timeout` に戻した（**呼び出し側が既にその意味で使っていた**）。`test_every_alive_call_in_the_repository_matches_the_signature` が `inspect.signature` と全呼び出しのキーワードを突き合わせる——検証スクリプトはどのゲートも import しないので、署名の不一致は走らせるまで分からない。変異（引数名を当時のものに戻す）で 2 件落ちる |
| W50 | 失敗の無い検証実行は、何かを検証している | **全件 SKIP は pytest の終了コード 0 であり、`make verify` は成功と報告する。** EX-S02 を書いている最中に起きた——sed で `s01-` を `s02-` に置換したとき、uart 名だけでなく `build-comm-s01-sat0` という**ビルド名まで**書き換わり、存在しないディレクトリを指した。`pytestmark` の skipif が全 5 件を飛ばし、`make verify` は「5 skipped」と出して**成功した**。同じ置換ミスをその 30 分前にシナリオ側でもやっており、そちらは Renode の `-e` 連鎖が止まってコンソールが空になる——**片方は無言の失敗、片方は無言の偽の成功**だった | `conftest.py` に `pytest_sessionfinish` を足し、**collected が全部 skip なら終了コードを落とす**（理由の先頭 3 件も印字する）。skip は「まだビルドしていない」への正当な答えだが、**全部への答えではない**。あわせて `test_every_build_directory_named_anywhere_is_one_the_makefile_produces` を追加し、Makefile が書くビルド名の集合と、シナリオ・検証スクリプト・`tests/e2e` が名指しする集合を比べる。**両方の置換ミスを再現して両方落ちることを確認した**——最初の版は verify 側を捕まえられず、`build-x/zephyr` の形しか見ていなかった（Python のパス連結にはスラッシュが無い）|
| W51 | テレコマンドに使ったノンス構成は、そのままテレメトリにも使える | **同じ鍵で同じノンスが出る。** EX-S02 のノンスは `APID(2) | 相手方 id(2) | seq(4) | zeros(4)` で、テレコマンドの範囲では正しかった。EX-D01 で**報告に同じ構成を使った瞬間**、ある局**から**の指令（seq 5）とその局**宛**の報告（seq 5）が**同一のノンス**になる。AES-GCM はノンス再利用に対し、平文ではなく**認証副鍵**を漏らす——つまり「再送検査が守るもの」ではなく**暗号そのもの**が壊れる | 方向を 1 オクテット入れて分離（`direction(1)`）。**実装より前に気づいたのは、2 つ目の用途を書いたからであって、レビューしたからではない。** 相手方 id の読み出し位置も方向で変わる（TC は副ヘッダ +3、TM は +5）ので、`test_both_read_the_counterparty_from_the_right_place` が両実装の一致を照合する。`test_the_c_signer_and_the_python_signer_produce_the_same_packet` は、C に準備させ・C が言う範囲に MAC を書き・結果が Python の `sign` と一致することを 1 つの assert で確かめる——長さ書き換え・シーケンス位置・ノンス・認証範囲の 4 つが同時に照合される |
| W52 | 締切を書いたループは、締切まで待つ | **待たない。** `test_a_downlink_reaches_the_client_and_is_recorded` は 5 秒の締切を設けながら、受信ループが `except socket.timeout: break` で**最初のタイムアウトで諦めていた**。単独では常に通り、スイートが 609 件に育った負荷の下で **3 回に 1 回**落ちた。2026-09-14、EX-L02 を書いている最中に出た。**W48 と同じ形**——ゲートがリンクの運を報告し、次の人は読まずに再実行して緑にする | `continue` に直して締切を実際に守らせた。両方の発生箇所（単局と 2 局）を直し、**下りを転送しない変異で今も 2 件落ちる**ことを確認した——「待つようにしたら何も検査しなくなった」ではないことの確認である。フル実行 4 回連続で通過 |
| W53 | 新しいモジュールを足すとき、名前が空いているかは自明である | **空いていなかった。** EX-L03 のパススケジュールを `src/cuberange/gs/schedule.py` に書いたが、そこには **EX-G01 の指令計画ストア**（`ScheduleStore`、161 行）が既にあり、上書きした。**止めたのは EX-G01 自身のテストの ImportError** であって、新しいコードの側の何かではない——つまり網は**偶然**だった | `gs/passes.py` に移し、元を `git checkout` で復元。あわせて `test_every_module_is_imported_by_something_that_runs` を足し、`src/cuberange` の 26 モジュールすべてが、テスト・演習・ツールのどこかで名指しされていることを照合する（全部 import すると収集時に Renode やソケットサーバまで起きるので、**名指し**で確認する）。誰も名指ししないモジュールを 1 本足す変異で落ちることを確認した。**偶然の網を、意図的な網にした** |
| W54 | ペアゲートの「1 フラグだけ違う」は、そのフラグが効いていることを含意する | **含意しない。** 検査 5 は 2 つのコンパイル行の**対称差が宣言した `-D` 以外に何も無いこと**を見る。フラグがどちらのビルドにも**無ければ対称差は空**であり、通る。2026-09-14、EX-U01 の `CUBERANGE_OBC_ACK_COMMANDS` をソースとビルドコマンドに足して `target_compile_definitions` に足し忘れたとき、ゲートは**6 項目すべて PASS** を出し、証拠行は「3 TU が -DCUBERANGE_OBC_ACK_COMMANDS 以外同一」と述べた。**実際に運んでいた翻訳単位は 0 である。** 検査 3（ELF が異なる）も助けにならなかった——ELF はフラグと無関係の理由で異なっていた。**不等性の主張は性質の主張ではない** | 検査 6 を追加: 各半分の**少なくとも 1 つの翻訳単位が、宣言した値でその `-D` を実際に運ぶ**こと。実物の EX-U01 で落ちることを確認し、`target_compile_definitions` を直して 13 ペア全部が通ることを確認。自己テストにも既知不良ケース (h)「フラグがどの翻訳単位にも届かない」を足した——**落ちないゲートは証拠ではない**という、このファイル自身の規則である |
| W55 | 「侵入者が12個送ったのだから、検出器は12と読む」 | **読まない。8 である。** `unexplained_commands` は *聞いた数 − 送った数* という**差引**であり、侵入者の12個がリンクの唯一のアンチリプレイカウンタを進めた結果、運用者の次の4個が COMM にリンク層で拒否され、宇宙機に**聞かれなかった**。16 聞いて 8 送った、差引 +8。**侵入が、自分を打ち消す遮断を引き起こした。** 2026-09-15、実測で落ちた——検出器の定義から書いたアサーションであって、**読みから書いたものではなかった** | アサーションを `PROBES - COMMANDS` に直し、理由を演習の README と mitigation の両方に書いた。さらに `commands_heard` を公開し、差引と並べて出すようにした——**差引は自分の項を隠す**ので、+8 を小さな侵入と読む運用者は 3 分の 1 過小評価する |
| W56 | ベースラインはいつ取っても同じ | **同じではない。** 最初のビーコンは局が送信を始めた**後**に届きうる。そのとき原点は 1 指令ぶんずれ、`commands_heard` は真値 16 に対して **15** を返した。数え落としではなく、**原点が動いた**のである。2026-09-15 実測 | `_acquire()` を追加: **送信前に受信し**、ベースラインが立つまで待つ。実運用のパスが信号捕捉から始まるのと同じ順序であり、同時に算術を決定的にする。W48・W52 と同じ家系——**タイミングを含む量を、タイミングを決めずに測っていた** |
| W57 | `K_THREAD_DEFINE` で定義したスレッドは動く | **動かない。** 最後の引数が `K_TICKS_FOREVER` のとき、スレッドは**存在するが走らない**。`k_thread_start()` を呼ぶまでである。2026-09-16、EX-U03 の COMM は 20 件の拒否を**正しく数え**、**1 件も報告しなかった**。カウンタも、送信関数も、CSP 経路も、ビーコンの拡張も、地上の読み取りも**全部正しかった**——起動行だけが無かった。症状は**脆弱側ビルドと完全に同一**であり、つまりペアゲートを通り、演習も通り、『対策が効いていない』ではなく『対策とは何も違わない』に見えた | `k_thread_start(stats_id)` を追加し、**その理由をソースに書いた**。W47 と同じ家系——**ブートし、印字し、自己テストに通り、そして仕事の一つを黙ってやめるノード**。検証側の `test_the_radio_reports_what_it_threw_away` は `link_frames_refused == 20` を要求するので、これが**再発したら落ちる**。**見つけたのは測定であって、コードを読むことではなかった** |
| W58 | `wait_for_uplink` が返ったなら、フレームは衛星に届いている | **届いていない。** `wait_for_uplink` は**プロキシがフレームを記録した**時点で返る。衛星への転送は同じスレッドの**別の書き込み**であり、まだ起きていないことがある。`test_replay_transmits_the_captured_octets_and_not_a_re_encoding` は記録の直後に `before` を取っていたので、負荷の高い瞬間には `sat.received[before:]` が**元のフレームとリプレイの両方**を含み、比較が落ちた。2026-09-16、数百回通っていたスイートで**1回**落ちた——**最悪の種類の不安定さ**である | `before` を取る前に `sat.wait_for(len(wrap(frame)))` を置いた。**アサーションが依存する事象そのもの**を待つ。W48・W52 と同じ誤り——**隣接する事象を待っていた**。直したあと、`replay` を再符号化に変異させてテストが落ちることを確認し（落ちないゲートは証拠ではない）、11 テスト × 5 回を通した |
| W59 | `make verify EX=...` が通る検証器は `make verify-all` でも通る | **通らない。** 前者は演習ごとに**別プロセス**、後者は**1 セッション**である。`sys.modules` は共有なので、自分のディレクトリを `sys.path` に入れて `from solve import ...` した検証器のうち、**最初に import した者が勝ち**、後続は全員その `solve` を黙って受け取る。2026-09-16、EX-U02 と EX-U03 の両方がそれをやり、`make verify EX=` では両方 4/4・5/5 で通り、CI の 25 分目で `verify-all` が ImportError で停止した。**騒がしい側で済んだのは運である**——静かな側は、両方の solver が同じ名前を定義していた場合で、**2 番目の検証器が 1 番目の演習のコードを測って PASS する** | 両方を `importlib.util.spec_from_file_location` で**固有名のパス読み**に変更。ガード 2 本: (1) 演習が自分のディレクトリを `sys.path` に入れることを禁じる——全演習に `solve.py` があるので、素の import は**構造的に曖昧**である;(2) 全検証器を**1 インタプリタに順番に import** し、25 分後ではなく 0.2 秒で衝突を落とす。両方とも実物の衝突を戻して落ちることを確認した |
| W60 | 鍵交換が終わったかどうかは、新しい鍵で指令が通ることで確かめられる | **確かめられない。** 交換には二つの半分がある——新 SA の**有効化**と旧 SA の**無効化**——そして**症状があるのは前半だけ**である。2026-09-16 実測: 旧 SA を無効化した宇宙機と、有効のまま残した宇宙機は、**古い association で送信しない運用者にとって完全に同一**である（4受理・拒否0・両コンソールに何も無し）。**有効化は自ら名乗り、無効化は誰も送信しない方向の沈黙である** | EX-S03 を**ネガティブ試験**として作った——退役させた SA で意図的に送信する。そして無線が拒否した SPI を報告するようにした: 「4フレーム拒否」はリンク不良とも当て推量とも両立するが、「**SPI 9 が拒否された**」は一つのことしか意味しない。拒否理由も4種に分けた（不正形式／該当 SA 無し／**無効化済み**／MAC 不一致）——**運用者を4つの別の場所へ送るから**である |
| W61 | ペアの対策側は「新しい鍵だけを積む」でよい | **よくない。** 新しい鍵だけのビルドは、古い association を**拒否する**宇宙機ではなく**話せない**宇宙機である。単一フレームからは同じに見え、**制御としては別物**であり、ペアゲートはどちらでも「対策」と呼ぶ。W54 の形——**ゲートは差分を見て、差分の意味を見ない** | 両半分に **SA 表と両方の鍵**を積み、差分を `operational` の**1 ビット**だけにした。検証側は旧 SA が **DEACTIVATED という理由で**拒否されたことを要求する——「拒否された」だけでは、鍵を持たない宇宙機でも通ってしまう |
| W62 | `printk` の文言はファームウェアの内部事情である | **インタフェースである。** 2026-09-16、EX-S03 で COMM の拒否メッセージを 4 つの具体的な理由に分割したところ、**8 箇所のアサーション**（演習 2 本に 4、tests/e2e に 4）が古い文言を探していた。CI の 20 分目（`verify-all`）・通算 47 分目で **3 つが落ち**、残る **5 つは `not in` 形**——**改名されると無条件に通る**。EX-S01 のネガティブ検査は、どのビルドも出さない文字列を探して**何も主張せず成功を報告していた** | ガードを追加: 検証器が探すコンソール文字列を **AST で収集**し、4 文字以上の語が`firmware/` に**単語として**存在することを要求する（`authenticate` が `authenticated` で満たされないことが要点）。**両方向に変異させて限界も測った**——存在しない語は落ち、語彙を再利用した改名は**通る**。`%s` を厳密にモデル化すると *任意の文字列*が`"COMM rail %s"` の描画の部分文字列になるので**検査が空になる**。**取れる最強の形が、元の失敗より弱いことを書き残す** |

W44 は W42 の続きだが、こちらは**部分的に本当**だった分だけ質が悪い。
「fuzz にかけてある」（W42）は全体が偽で、走らせれば無いことが分かる。
「バイト単位で突き合わせてある」は走らせれば通る——通った試験が
**API の 3 分の 1 を見ていない**ことは、通った事実からは分からない。
主張が嘘になるのは書いた時点ではなく、API が増えた時点である。
だから今回入れた検査は復号器の比較そのものではなく、**ヘッダと試験の差分**を見るものにした。
次に誰かが `cr_*` を足したとき、この行が再び嘘になる前に落ちる。

同じ監査で、CONTRIBUTING.md の「ペアを手で `diff` して確かめよ」という指示が
`tools/config_diff_gate.py`（6 項目を機械検査し `make check` に入っている）に置き換わったまま
**英語版だけ**残っていたことも分かった。日本語版は更新済みだった。
文書間の対応を構造だけで比べる検査（`test_doc_parity.py`）は、見出しも表の行も箇条書きも
一致していれば通す——**読者が打つコマンドが両言語で別物でも通す**。

そこでコードブロック内のコマンドも照合するようにした。コマンドは文書のうち翻訳されない部分である。
散文は訳す、コメントも訳す、プレースホルダも訳す。`make pair-gate` は訳さない。
抽出は「PATH に実在するか、リポジトリ内の実行可能ファイルか」を**実際に引いて**判定する
（許可リストにすると、新しい道具が文書に出るたび誰かが気づくまで素通りする）。
これで演習 README の ASCII 図——`COMM  OBC  EPS  attacker  ------  CAN ハブ1本`、`you are here`——が
除かれる。等幅なだけの散文であり、`COMM` も `you` も PATH に無い。

導入した時点で 24 対中 2 対が落ちた。CONTRIBUTING.md と、EX-B01 の `mitigation.md` である。
後者は英語版だけ手動 `diff` のままで、さらに **`Only the length check changes` と書いてあった**——
EX-B01 のフラグは `CUBERANGE_EPS_REQUIRE_AUTH` で、長さ検査は EX-F01 のものである。
そして両言語がゲート出力として引用していた行数（4 行）は、検査 0 と検査 4 が後から増えた分だけ
古かった。実出力は 6 行。**引用も測定値であり、古くなる。**

W45 と W46 は同じ週に、同じ演習を作りながら出た。どちらも「無いこと」を「効いたこと」と
取り違える形をしている。

W45 では、**リンクが繋がっていないこと**が**攻撃が失敗したこと**に見えた。
W46 では、**誰も歩いていない動線**が**通っている動線**に見えた。
EX-G04 は地上局についてこれを言う——拒否と不達は地上から区別が付かない——のだが、
その文を書いた同じ range が、自分の配線と自分の文書で 2 回やった。

W46 は §16 の中で最も高くつく種類である。**誰も走らせていなかったのは、製品の一次動線だった。**
`make check` は 10 個のターゲットを回す。pytest、native、ファームウェア一式、demo-p0、
determinism、constellation、fleet、ground-stations、verify-all、pair-gate。
`make exercise` は 1 つも通らない。`make verify` は pytest の内側から Renode を起動し、
その pytest は既に名前空間の中にいる——**試験した経路と、文書が書いた経路は、
シナリオファイルを共有するだけの別経路だった**。

W30・W31 は「ゲートの入力を疑っていなかった」話だった。W46 はその一段手前で、
**ゲートが読者の道を一度も歩いていなかった**話である。verify-all は 36 のアサーションを
実測している。そのどれ一つとして、学習者が最初に打つ 2 行を実行していない。

調べ始めたときは `make exercise` 1 本の問題に見えた。実際には `make channel` も `make gs` も
それぞれ `$(ISOLATE)` を掛けており、EX-G03 と EX-G04 が指示する「端末 3 つ」は
**互いに見えない range 3 つ**を立ち上げる手順だった。修正は 1 箇所で済む——
`isolate` を**冪等**にする。既にループバック専用名前空間にいるなら、作らずに exec する。

これは 2 つの理由で正しい。生のバックエンドはこのホストで入れ子にできない（実測: bwrap in
bwrap は "No permissions to create a new namespace"、unshare は EPERM、いずれも
`kernel.apparmor_restrict_unprivileged_userns=1`）。そして入れ子にできる環境でも、
**入れ子の名前空間は別のループバックである**——繋がらない理由が「作れなかった」から
「作れてしまった」に変わるだけで、症状は同じく「何も繋がらない」である。

`make workflow` は `check` の中で唯一 ISOLATE で包まないターゲットにした。包んでも
（冪等になった今は）動くが、**この試験が試験したい名前空間は `make exercise` が作るもの**であり、
先に別のものへ入っては読者の経路ではなくなる。

W47 は §16 でいちばん気味の悪い形をしている。**自己テストは正しかった。** mbedTLS は公表 NIST
ベクタのタグを正しく計算し、そう印字した。その印字が出た時点で、宇宙リンクはもう死んでいた。

自己テストを置いたのは正しい判断だった——EX-G04 の教訓（拒否と不達は区別が付かない）を
暗号層に持ち込めば、鍵違い・レイアウト違い・リンク断はすべて「全部拒否される」という
同じ観測になる。だから答えを 1 行印字させた。そして**その 1 行を印字する処理が、リンクを壊した**。
自己テストは自分について正しく、自分以外について何も言わず、そして自分以外を壊した。

5 つの仮説を順に殺す作業に価値があったのは、どれも「もっともらしい」からである。
最後に残ったのは**測った数字が 2 つ並んだとき**だった——424 と 1024。
それより前に、推測でスタックを上げて直すことはできた。直したあとで、
**何が原因だったかを知らないまま**先に進むことになっていた。

W48 はその同じ CI 実行が見せたもう 1 つで、W47 と組で読む価値がある。
W47 は**無言の失敗**、W48 は**無言の偽の失敗**である。前者は「何も起きない」を「対策が効いた」に
見せ、後者は「1 フレーム落ちた」を「対策が壊れた」に見せる。どちらも、観測を主張にする段で
起きている——EX-G04 が地上局について言っていることが、こちらでは**自分の試験**について起きた。

W49 は W48 の修正そのものが出した。**間欠故障を直す変更が、決定的な故障を 3 つ作った**——
そしてその 3 つは、ゲートが検査している対象（対策が壊れること）と同じ見た目で報告された。

ここで効いたのは CI である。ローカルの `make check` の pytest は通り、`verify-all` も通したが、
`alive(timeout=...)` を**実際に評価する**のは Renode を起こす経路だけで、
その経路を「関係ない変更だから」と省いた。省いた理由が間違っていたのではなく、
**省いたこと自体が測定の不在**だった。

W48 と W49 を並べると、もう 1 つ見える。否定の主張——`not alive(timeout=8)`、
「攻撃のあと衛星は応答しなくなった」——は肯定より**強い**主張である。
単発 ping ではそれが安く済み、そして間違っていた。1 フレーム落ちれば攻撃成功と読めてしまう。
リトライにすると予算を使い切るようになり、そして正しくなった。EX-L01 の実行時間が
45 秒から 66 秒に伸びたのは、**主張が強くなった分の代金**である。

W50 は W42・W46 と同じ家族で、いちばん小さく、いちばん再発しやすい形である。
W42 は存在しない fuzzer、W46 は誰も歩いていない動線、W50 は**走らなかった試験が出す緑**である。
3 つとも「機構が無いこと」を「機構が働いたこと」と取り違えており、3 つとも
**取り違えた側が沈黙で表現される**という共通点を持つ。

そして W50 を出したのは、同じ 30 分のうちに 2 回やった同じ置換ミスだった。
`sed 's/s01-/s02-/g'` は uart 名を直すつもりで、ビルド名も直した。
1 回目（シナリオ）は Renode が止まってコンソールが空になり、**失敗に見えた**。
2 回目（検証スクリプト）は全件 skip で、**成功に見えた**。
同じ間違いが、当たった場所によって反対の見た目をする——だから両方をゲートにした。
片方だけ塞げば、次は必ずもう片方で起きる。

W51 は §16 の中で唯一、**出荷前に捕まえたもの**である。だから書き方が違う——
「こう間違えた」ではなく「**こう間違えかけ、何が気づかせたか**」。

気づかせたのは、2 つ目の用途を書いたことだった。レビューではない。
ノンス構成を 1 つ目の用途で設計したとき、それは正しかった。
**正しさが用途に依存していることは、用途が 1 つの間は見えない。**

そして GCM を選んだこと自体がこの罠を鋭くしている。多くの構成でノンス再利用は
「その 2 メッセージの機密性が落ちる」で済む。GCM では**認証副鍵が漏れ**、以後すべての偽造が可能になる。
「再送カウンタは可用性の話」と思っていると、**暗号そのものの前提だったことを見落とす**。
`pus_auth.py`・`cuberange_pus_auth.h`・ゴールデンファイルの 3 箇所に同じ文があるのは、
将来カウンタを緩める変更が、再送検査ではなく**暗号を**壊すからである。

EX-D02 はその裏返しを演習にしたものである。報告カウンタは P0 の初日から全 PUS TM 副ヘッダに
入っていた。**読まれた日に役に立つようになり、それ以前ではなかった。**
そして読み方には順序がある——認証してから数える。逆順は、攻撃者が書いたオクテットを数える。

W52 は W48 の再発である。場所が違うだけで形は同じ——**締切を書き、守らないループ**。
W48 は「ping 1 回で生死を決める」、W52 は「5 秒待つと書いて 0.2 秒でやめる」。

再発したこと自体が情報である。W48 のときに直したのは**呼び出し側 5 本**であって、
「締切を守らない待ち」という**形**を探しには行かなかった。
形を探していれば、このファイルの 2 箇所は同じ日に見つかっていた。

そして露出したのは、スイートが 609 件に育って負荷が上がったからである。
**フレークは負荷の関数であってコードの関数ではない**——
「今は通っている」は「正しい」ではなく「まだ軽い」である。

EX-L02 の由来も同じ形をしている。`link_channel.py` は最初から
「伝搬遅延・ビット誤り・パス窓・ドップラーは模擬していない」と書いていた。**真であり、開示だった。**
EX-D02 はその上に検出器を建て、自分の対策文書に
「実リンクでは誤検知する。この range にビット誤りが無いので、ここでの欠落はすべて誰かの判断である」
と書いた。**これも真だった。** 真であることと、読者がそれを学ぶことは別である。
脚注の限界は脚注の限界でしかない——この range の答えは最初から
「**次の演習が前の演習の限界を攻める**」であり、EX-L02 は、
その限界が敵ではなく**環境**だった場合である。

W53 は小さく、そして §16 で最も**運が良かった**ものである。161 行のモジュールを上書きし、
気づかせたのは**関係のないテストが偶然その名前を import していたこと**だった。
その import が無ければ、EX-G01 は壊れたまま残り、CI の verify-all で落ちるのは
**数十分後**——変更セットが十分に大きくなってからである。

だから足したガードは「上書きを防ぐ」ものではない（それは道具の仕事である）。
「**どのモジュールにも、壊れたら気づく場所がある**」を確かめるものである。
偶然そうだったことを、そうであり続けるようにした。

EX-L03 そのものも同じ形の話をしている。スケジュールという地上側の検出は、
**宇宙機が訊かれずに喋る**という前提の上に立っている。前提が無ければ毎パス発火する。
EX-D02 の検出器は入力が**偽造可能**で開いて失敗し、EX-L03 のそれは入力が**存在せず**閉じて失敗する。
**あらゆるものに発火する検出器と、何にも発火しない検出器は、服の違う同じ欠陥である。**

W54 は §16 の中で**ゲート自身**に見つかった 2 件目であり、形は W30・W31 と同じである——
検査は正しい性質を見ていて、**その性質の主語が存在するかを誰も問わなかった**。

検査 5 は「他に差が無い」を証明する。「宣言した差がある」は証明しない。
2 つは別の主張であり、前者だけを書いたとき、**空の差分**という最も静かな入力が通る。
そして証拠行は前者の言葉で後者を報告する——
「3 TU が -D 以外同一」は真であり、読む人が受け取るのは「-D が 3 TU で効いている」である。

これを見つけたのは実装ではなく**測定**だった。EX-U01 の 4 通りを測って
「hard・攻撃なし」でも受理応答が 0 だったところから辿った。
ゲートは 6 項目 PASS を出していたので、**ゲートを信じていれば止まっていた**。

W55 は §16 で初めて、**検出器そのものの読み方**を間違えた記録である。
カウンタは正しく数え、ゲートは正しく落ち、書き手だけが数の**意味**を定義から推測していた。
差引は引き算であり、引き算には項が 2 つある。片方しか見ない読み手には、
**攻撃が小さく見える**——しかも、攻撃が大きいほど打ち消しも大きくなるので、**過小評価は攻撃に比例して増える**。

そして W55 を見つけたのは、EX-U02 を作る過程で**測ったから**である。
12 と書いたアサーションが 8 で落ちた。落ちなければ README に 12 と書いていた。
**このプロジェクトの規則は「走らせていない主張を書くな」であり、W55 は規則が仕事をした記録の側にある。**

W57 は W47 の 4 回目であり、形は完全に同じである——
**機能の全部品が正しく、それらを走らせる 1 行が無い。**
W47 はスタックが足りずスレッドが死んだ。W57 はスレッドが始まらなかった。
どちらも「ブートし、印字し、自己テストに通り、そして仕事の一つを黙ってやめる」ノードを作る。

**そして今回は、症状が『脆弱側と同一』だった**という点が新しい。
壊れた対策ビルドは、ペアゲートを通る（フラグは本当に翻訳単位に届いていた）。
演習も通る（攻撃は本当に拒否されていた）。
違いは**ゼロ**であり、それは EX-U03 が**教えている内容そのもの**である——
**完全に働いた制御と、何も無いことは、報告が無ければ区別がつかない。**
自分のビルドで、自分の演習の主題を踏んだ。

W59 は §16 で 3 度目の「**検証器が、自分が測っていると思っているものを測っていない**」である
（W30・W31 がゲート自身、W54 がペアゲート）。そして今回は**測定対象が別の演習**になりうる。

形を一般化すると、W50・W53・W59 は同じ問いに帰着する——
**このテストは、隣にあるものと一緒に走ったときも、同じことを測るか。**
W50 は全スキップが成功を返し、W53 は別モジュールを上書きし、W59 は別演習のコードを import した。
どれも**単独で走らせれば正しい**。`make verify EX=...` が通ったという事実は、
`verify-all` について**何も言っていない**。

W60 は §16 で初めて、**実施され検証された制御**についての記録である。
W1〜W59 はどれも「無かった」「報告できなかった」「測っていなかった」のいずれかだった。
W60 は**やった・確かめた・半分だった**であり、確かめが半分だったのは
**変更のうち音を立てる側を試験したから**である。

これが一般形である——
**制御を入れたことは、それが働くことの証拠になる。制御を切ったことは、何の証拠も生まない。**
無効化・失効・撤回・剥奪は、どれも「誰も試さない方向の沈黙」として実装され、
そして**沈黙は、まだ訊かれていないことと区別がつかない。**
