...
| W32 | 日本語版の文書が英語版と同じ内容を述べている | **3 つの文書が実質的にずれていた。** `ASSURANCE.ja.md` の表は PUS-C を「独立オラクル無し」、FECF を 2 つとし、参照先に存在しないテスト名を挙げ、さらに「ベクタは未コミット、消費するテストも無い」と書いていた——いずれも英語版では解消済み。`SECURITY.ja.md` は英語版が開示している意図的な脆弱性のうち **3 件を欠いていた**（EX-F01 の標的である特権ハンドラ、地上系の無検査インポート、平文の署名鍵）。`README.ja.md` は `make firmware-g01` を欠いていた。**検証状況を述べる文書と、脆弱性を開示する文書自身が、日本語読者に対して古い内容を述べていた。**見出し構造しか比較しない検査では 1 件も捕まらない | 3 文書を実態に合わせて書き直した。`test_doc_parity.py` に**表の行数**の比較を追加した——翻訳では本文を比較できないが、行の欠落は数えられる。実際に見つかった 2 件のドリフトはどちらも表の行だった。変異テストで、行を 1 つ落とすと落ちることを確認済み |
