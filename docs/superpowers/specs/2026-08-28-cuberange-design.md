...
そして隔離は**入れ子にできない**（実測: bwrap in bwrap は "No permissions to create a new
namespace"、unshare は EPERM、いずれも `kernel.apparmor_restrict_unprivileged_userns=1`）。
だから `make workflow` は `check` の中で唯一 ISOLATE で包まないターゲットになる。
包むと通らないのではなく、包むと**この試験が試験したい名前空間ではなくなる**。
