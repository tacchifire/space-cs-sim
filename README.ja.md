...
EX-G02 は意図的に6つ目の起点を足さない。その主題は、**正当な起点が自分の権限を超える**ことである。
...
実際のホストで**実行したもの**：`tools/ci.sh`。環境検査、その自己テスト、そして `make check` の通し実行である。
**実行していないもの**：ワークフロー自体。
GitHub Actions がこれを走らせるところを誰も見ていない。

ワークフローは検証済みコマンドを呼ぶだけの薄い層なので、未検証なのはゲートではなく周辺の配管である。
それでも、実行が観測されるまでの正直な説明は「**ゲートは今も人間が打つコマンドである**」であり、この段落はそれが変わるまでそう書き続ける。
